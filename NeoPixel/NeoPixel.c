...
uint16_t  ledPwmData_A[(24 * CANT_LEDS)+60] = {0};
uint16_t  ledPwmData_B[(24 * CANT_LEDS)+60] = {0};
uint16_t  ledPwmData_C[(24 * CANT_LEDS)+60] = {0};
uint16_t  ledPwmData_D[(24 * CANT_LEDS)+60] = {0};
uint16_t  ledPwmData_E[(24 * CANT_LEDS)+60] = {0};
...
{
    uint32_t ledData =  red | green <<8 | blue <<16;
    ...
    switch(channel)
    {
        case A:
            ...
            dma_channel_configure(pwm_dma_chan, &pwm_dma_chan_config, &pwm_hw->slice[slice_num].cc,
                ledPwmData_A,  (24 * CANT_LEDS)+60, true );
        break;
        case B:
             slice_num = pwm_gpio_to_slice_num(4);
            dma_channel_configure(pwm_dma_chan, &pwm_dma_chan_config, &pwm_hw->slice[slice_num].cc,
                ledPwmData_B,  (24 * CANT_LEDS)+60, true );           
        break;
        case C:
            slice_num = pwm_gpio_to_slice_num(5);
            dma_channel_configure(pwm_dma_chan, &pwm_dma_chan_config, &pwm_hw->slice[slice_num].cc,
                ledPwmData_C,  (24 * CANT_LEDS)+60, true );                 
        break;
        case D:
             slice_num = pwm_gpio_to_slice_num(20);
            dma_channel_configure(pwm_dma_chan, &pwm_dma_chan_config, &pwm_hw->slice[slice_num].cc,
                ledPwmData_D,  (24 * CANT_LEDS)+60, true );                
        break;  
        case E:
             slice_num = pwm_gpio_to_slice_num(21);
            dma_channel_configure(pwm_dma_chan, &pwm_dma_chan_config, &pwm_hw->slice[slice_num].cc,
                ledPwmData_E,  (24 * CANT_LEDS)+60, true );                
        break;               
    }    
